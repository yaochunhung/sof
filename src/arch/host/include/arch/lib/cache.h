...
#define TESTBENCH_BACTRACE_SIZE		1024
...
struct tb_cache_entry {
	...
	/* backtrace info */
	void *symbols[TESTBENCH_BACTRACE_SIZE];	/* last stack usage */
...
{
	void *backtrace_data[TESTBENCH_BACTRACE_SIZE];
	int backtrace_size;

	backtrace_size = backtrace(backtrace_data, TESTBENCH_BACTRACE_SIZE);
...
{
	if (req_size % TESTBENCH_CACHE_LINE_SIZE)
		return req_size + TESTBENCH_CACHE_LINE_SIZE -
			(req_size % TESTBENCH_CACHE_LINE_SIZE);
	else
		return req_size;
}
...
static inline long _cache_op_offset(void *base, void *addr)
{
	...
	if (offset % TESTBENCH_CACHE_LINE_SIZE)
